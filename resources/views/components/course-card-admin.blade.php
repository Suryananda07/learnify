<div class="py-4 px-5 flex flex-col gap-3 justify-between h-full bg-[#F6F2FF] border-2 border-[#8650FF] rounded-2xl">
    <div class="flex flex-col gap-3">
        ...
        <p class="text-sm sm:text-base text-slate-600 wrap-break-word whitespace-pre-line">{{ $course->description }}</p>
    </div>
    <div class="flex gap-2.5 sm:gap-4">
        <a href="{{ route('courses.show', $course->id) }}" class="text-primary text-base flex gap-1.5 items-center"><svg
        ...
        <a href="{{ route('courses.edit', $course->id) }}" class="text-primary text-base flex gap-1.5 items-center"><svg
        ...
        <form action="{{ route('courses.destroy', $course->id) }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-primary text-base flex gap-1.5 items-center"><svg xmlns="http://www.w3.org/2000/svg"
